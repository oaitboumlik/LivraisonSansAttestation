...
def concat_meteo_traffic(meteo_path, df_traff, delimiter=';'):
    df_meteo = pd.read_csv(meteo_path, delimiter=delimiter)
    date_col = "datetime_meteo"
    df_meteo.rename(columns={'Date': date_col}, inplace=True)
    df_meteo[date_col] = df_meteo[date_col].map(lambda x: pd.to_datetime(x, format="%Y-%m-%d %H:%M:%S"))
    df_traff[date_col] = df_traff['datetime'].map(lambda x: pd.to_datetime(str(x)[:-6], format="%Y-%m-%dT%H:%M:%S"))

    df_merge = pd.merge(df_meteo, df_traff, on=date_col, how='right')
